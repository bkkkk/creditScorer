...
levels(imputed_tr$seriousDlqin) <- c("Paid", "Defaulted")

upSampled <- upSample(x = imputed_tr %>% select(-seriousDlqin), y = imputed_tr$seriousDlqin)

model_fixed <- train(
  x = as.matrix(upSampled %>% select(-Class)),
  y = upSampled$Class,
  method = "ranger",
  trControl = trainControl,
  verbose = TRUE,
...
knitr::kable(model_fixed$finalModel$confusion.matrix)
confusionMatrix(model_fixed$finalModel$confusion.matrix)
model_with_prob
```
...
importance_list <- data.frame(model_fixed$finalModel$variable.importance)
...
preds <- predict(model_fixed$finalModel, imputed_te)

preds_col = preds$predictions
level(preds_col) <- c("Paid", "Defaulted")

predicted_te <- imputed_te %>% mutate(seriousDlqin = preds_col) %>% select(seriousDlqin, everything()) %>% mutate()
```
